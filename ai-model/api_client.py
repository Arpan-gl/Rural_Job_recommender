...
# async def fetch_indianapi_jobs(query: JobQueryStructured, client: httpx.AsyncClient) -> List[JobListing]:
    """
    Fetches jobs from the IndianAPI.in Jobs API.
    Requires: INDIANAPI_JOBS_KEY
    Docs: Provided by user (indianapi.in/jobs-api)
    """
    print("[API Client] Querying IndianAPI.in Jobs (Key Required)...")
    jobs_list: List[JobListing] = []
    api_key = os.getenv("INDIANAPI_JOBS_KEY")

    if not api_key:
        print("[API Client] INDIANAPI_JOBS_KEY not set. Skipping IndianAPI.in Jobs.")
        return []

    base_url = "https://jobs.indianapi.in/jobs"
    headers = {"X-Api-Key": api_key}
    
    # --- Map structured query to API parameters ---
    params = {"limit": "15"} # Default limit
    
    if query.locations:
        params["location"] = query.locations[0] # Use first location
        
    # Use skills or job titles for the 'title' parameter
    if query.skills:
        params["title"] = query.skills[0] 
    elif query.job_titles:
         params["title"] = query.job_titles[0]
         
    if query.experience_level:
        # Map experience level if possible (API expects 'Fresher', '1-3 years', etc.)
        if query.experience_level == 'entry-level':
            params["experience"] = "Fresher" 
        # Add more mappings if needed based on API's expected values
        
    # Example: Add job_type mapping if your LLM extracts it
    # if 'job_type' in structured_query_dict and structured_query_dict['job_type']:
    #    params["job_type"] = structured_query_dict['job_type']

    try:
        response = await client.get(base_url, params=params, headers=headers, timeout=15.0)
        
        if response.status_code == 401:
            print("[API Client ERROR - IndianAPI.in] Unauthorized. Check your INDIANAPI_JOBS_KEY.")
            return []
        if response.status_code == 422:
             print(f"[API Client ERROR - IndianAPI.in] Validation Error (422): {response.text}")
             return []
             
        response.raise_for_status() # Raise errors for other non-2xx codes
        
        data = response.json() # Response is a list of job objects
        ...
        for item in data:
            jobs_list.append(JobListing(
                ...
                company=safe_get(item, 'company'),
                location=safe_get(item, 'location'),
                url=safe_get(item, 'apply_link', '#'),
                source="IndianAPI.in",
                # Combine relevant fields for the snippet
                description_snippet=(
                    f"Type: {safe_get(item, 'job_type')}, Exp: {safe_get(item, 'experience')}. "
                    f"{safe_get(item, 'job_description', '')[:150]}..."
                )
            ))
            
        print(f"[API Client] Found {len(jobs_list)} jobs from IndianAPI.in.")

    except httpx.RequestError as e:
        print(f"[API Client ERROR - IndianAPI.in] Request failed: {e}")
    except Exception as e:
        print(f"[API Client PARSE ERROR - IndianAPI.in] Failed to process response: {e}")
...
async def scrape_all_platforms(query: JobQueryStructured) -> List[JobListing]:
    ...
    async with httpx.AsyncClient() as client:
        ...
        tasks = [
            ...
            # fetch_indianapi_jobs(query, client),
